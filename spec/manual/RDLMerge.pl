...
while(<FILE>)
{
    ...
    system("$java_version -jar Ordt.jar -parms openreg.parms -xml $OutDir/$sub_file_prefix.xml -verilog $OutDir/${sub_file_prefix}_reg.v -uvmregspkg $OutDir/ordt_uvm_reg_pkg.sv -uvmregs $OutDir/${sub_file_prefix}_reg.sv -cppmod $OutDir/${sub_file_prefix}_reg_c $sub_filename") == 0 or die $!;
    # TODO
...
system("$java_version -jar Ordt.jar -parms openreg.parms -xml $OutDir/openreg.xml -verilog $OutDir/openreg_reg.v -uvmregs $OutDir/openreg_reg.sv -cppmod $OutDir/openreg_reg_c $OutDir/openreg.rdl") == 0 or die $!;
