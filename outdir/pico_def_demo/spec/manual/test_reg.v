...
(
  ...
  always @ (posedge clk or posedge sig_stats_another_reset) begin
...
(
  ...
  input    clk;
  input    reset;
  input     [14:2] h2d_pio_dec_address;
  input     [127:0] h2d_pio_dec_write_data;
  input     [1:0] h2d_pio_dec_trans_size;
  ...
  //------- outputs
  output     [1:0] d2h_dec_pio_trans_size;
  output     [127:0] d2h_dec_pio_read_data;
  ...
  reg  h2d_pio_dec_read_lvl;
  reg  h2d_pio_dec_write_lvl;
  ...
  assign  pio_dec_read = h2d_pio_dec_read_lvl;
  assign  pio_dec_write = h2d_pio_dec_write_lvl;
  ...
  //------- reg assigns for parallel pulsed i/f signals
  always @ (posedge clk or posedge reset) begin
    if (reset) begin
      h2d_pio_dec_read_lvl <= #1  1'b0;
      h2d_pio_dec_write_lvl <= #1  1'b0;
    end
    else begin
      h2d_pio_dec_read_lvl <= #1  h2d_pio_dec_read | (h2d_pio_dec_read_lvl & ~(dec_pio_ack | dec_pio_nack));
      h2d_pio_dec_write_lvl <= #1  h2d_pio_dec_write | (h2d_pio_dec_write_lvl & ~(dec_pio_ack | dec_pio_nack));
    end
  end
